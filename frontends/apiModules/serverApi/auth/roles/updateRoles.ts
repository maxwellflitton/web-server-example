 ...
 * - `roles`: an array of roles that are going to be assigned to the user.
 * - `jwt`: User's jwt.
 */
const updateRolesInputSchema = z.object({
  user_id: z.number(),
  roles: z.array(
    z.enum([
      "Super Admin",
 ...
 */
const updateRolesOutputSchema = z.object({}).strict();

// Export types inferred from the Zod schemas.
export type UpdateRolesInputSchema = z.infer<typeof updateRolesInputSchema>;
export type UpdateRolesOutputSchema = z.infer<typeof updateRolesOutputSchema>;

/**
 * Type definition for the function that creates a new user.
 */
export type UpdateRolesFunction = ApiFunction<UpdateRolesInputSchema, UpdateRolesOutputSchema>;
 ...
 *
 */
export async function updateRoles(input_data: UpdateRolesInputSchema): Promise<ApiResponse<UpdateRolesOutputSchema>> {
  const body = { user_id: input_data.user_id, roles: input_data.roles };

  const url = new RoleUrl().updateUserRoles;
  ...
  if (response.status === 200) {
    const parsedOutput = updateRolesOutputSchema.safeParse(response.body);
