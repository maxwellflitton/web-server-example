//! Core logic for resending a confirmation email
...
pub async fn resend_confirmation_email<X, Y, Z>(email: String) -> Result<(), NanoServiceError> 
where
    ...
    Z: GetConfigVariable,
{
    ...
    match send_confirmation_email::<X, Y, Z>(email.clone(), new_uuid.clone()).await {
        Ok(outcome) => {
            if outcome == false {
                return Err(NanoServiceError::new("Failed to resend confirmation email due to a rate limit error".to_string(), NanoServiceErrorStatus::Unknown))
            }
            Ok(())
        },
        Err(e) => Err(e)
...
mod tests {
    ...
    #[tokio::test]
    async fn test_resend_confirmation_email() {
        // Test success
        reset_flags();
        let result = resend_confirmation_email::<MockDbHandleSuccess, MockMailchimpHandleOk, FakeConfig>(
        ...
        reset_flags();
        let result = resend_confirmation_email::<MockDbHandleSuccess, MockMailchimpHandleReturnFalse, FakeConfig>(
            "example@gmail.com".to_string(),
        )
        .await;
        assert!(result.is_err());
        ...
        assert_eq!(
            error.message,
            "Failed to resend confirmation email due to a rate limit error"
        );
        ...
        reset_flags();
        let result = resend_confirmation_email::<MockDbHandleSuccess, MockMailchimpHandleError, FakeConfig>(
            "example@gmail.com".to_string(),
        )
        .await;
        assert!(result.is_err());
